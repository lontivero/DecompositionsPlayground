...
namespace X
{
	public class Program
	{
		...
		public void Main()
		{
			...
			var target = 8; //(long)(1.2345 * 100_000_000);
			stdDenomsInAcceptableRange = StdDenoms.Where(x => x > Dust && x <= target).Reverse().ToArray().AsSpan();
			var decompositionTree = Decompose(target, decompositionsCache);

			var decomposition = (Factor)decompositionTree;
			foreach(var d in Flatten(decomposition, MaxRecursionDepth))
			{
		...
		private static List<List<long>> Flatten(Factor n, int depth, long parent = long.MaxValue)
		{
			var alternativeCount = n.Alternatives.Count;
			var altResults = new List<List<long>>();
//			if (depth == 0)
//			{
//				altResults.Add(new List<long> { n.Value });
//				return altResults;
//			}
			...
			foreach (var alt in n.Alternatives)
			{
				...
				if (depth > 0)
				{
				var flattenRight = Flatten(alt.Right, depth - 1, fst);
		...
		private static void FillDecompositionsCache(SortedList<long, Factor> cache, Span<long> stdDenomsInAcceptableRange)
		{
			...
			for (var i = 1; i < stdDenomsInAcceptableRange.Length; i++)
			{
				...
				var decomposition = Decompose(currentDenomination, cache);
			...
			SortedList<long, Factor> cache)
		{
			...
			if (target % 2 == 0)
			{
				if (cache.TryGetValue(target / 2, out var restDecomposition))
				{
					decompositions.Add(new Addition(restDecomposition, restDecomposition));
				}
			}
			if(target % 3 == 0)
			{
				if (cache.TryGetValue(target / 3, out var restDecomposition))
				{
					decompositions.Add(
						new Addition(
							Decompose(2 * restDecomposition.Value, cache),
							restDecomposition));
				}
			}
			if(target % 5 == 0)
			{
				if (cache.TryGetValue(target / 5, out var restDecomposition))
				{
					decompositions.Add(
						new Addition(
							Decompose(3 * restDecomposition.Value, cache),
							Decompose(2 * restDecomposition.Value, cache)));
				}
			}
